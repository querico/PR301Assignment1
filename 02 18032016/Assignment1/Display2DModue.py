...
class disPlay2D (object):
    ...
    def displayAgeGroupIncome():
        ...
        for element in range (0,list_size):
            if int(data_list[element][2]) > 0 and int(data_list[element][2]) <= 30:
                under_30_income += int(data_list[element][5])
            elif int(data_list[element][2]) > 30 and int(data_list[element][2])<=50:
    ...
    def displayBMIIncome():
        ...
        rects = plt.bar([index + bar_width for index in range(n_groups)], incomes, bar_width,
                alpha=opacity,
                color='r',
                yerr=incomes,
        ...
        plt.xlabel('BMI')
        plt.ylabel('Income')
        plt.xticks([index + bar_width for index in range(n_groups)], ('Normal', 'Overweight', 'Obesity', 'Underweight'))
        plt.show()
